...
### L-11 中断リラン + 無人運用ゲート (2026-08-23 午後・Takashi 立ち会い・**全項目合格**)

- 詳報: my-ksql-jobs `docs/scale_l11_report.md` / スクショ: Exit 5(ターミナル B)・Ctrl+C の瞬間・ログアプリ RUNNING(chunk:72)・BATCH+JOB RUNNING・as_of 継承(record 99/100)・webhook 受信 2 件
- **L-11 ①**(単発 run): Ctrl+C 中断(batch 0bf51f0b)→ ログアプリ RUNNING・CP chunk:72 vs **ローカル JSONL 実書込 chunk:81**(CP 遅延の実例)→ 即時再実行 exit 5 → unlock(一覧→解除)→ 同一 as-of リラン SUCCESS(20,000/20,000・API 259)→ MCP 突合 touched 20,000/20,000(重複・欠落なし)
- **L-11 ②**(run-all): Ctrl+C → BATCH+JOB 両方 RUNNING → unlock(2 件+ローカルロック)→ `--resume` が**「元セッションの as-of を引き継ぎます」をコンソール明示**・新バッチ as_of 同一のまま SUCCESS(API 265・528 秒)・中断側は TIMEOUT へ解消
- **ゲート 1**(二重起動): exit 5 を 3 回観測。**防御は二層** — プロセス生存中は`.ksql\lock-prod.json`(pid/host 表示)、消滅後はログアプリのジョブキー。別マシン多重起動もログアプリ層で防げる設計を実機確認
- **ゲート 2**(通知): webhook.site 受信 2 件 — 失敗通知(514B・exit_code/jobs[].error 全文/text 行)+ heartbeat(140B・軽量)。発火は `--max-api-calls 5` の安価な失敗(API 5)と新設 heartbeat_ping.sql(API 4)
- **ゲート 3**(実行環境): VSCode 外の新シェルで verify SUCCESS(68,036/147・環境変数の手動設定不要)
- サイクル API ≈ 1,450(seed 412 秒・リラン 474/528 秒・cleanup 62.8 秒 = 本日最速)
- 発見: ログアプリの**エンジン版数が「flow 0.1.0」表示**(実体 v0.2.0)— ランナーのバージョン文字列更新漏れ疑い(POST 系起票候補)
- **これで企画 R2 の全観点(1〜8・L-9〜L-13・無人運用ゲート)を実機消化 — スケール検証完了**。残: 仕様書 11 章更新・記事 #3 執筆

### XL-実機サイクル (2026-08-23・**全ステップ合格**)

- 詳報: my-ksql-jobs `docs/scale_xl_report.md`（全出力・batch_id 一覧・アプリ別内訳つき）/ スクショ: 撮影なし
- **主目的の結論（仕様 11 章の公表目標 vs 実測）**:
  - 時間「10 分以内」→ **中央値 200.7 秒（3.3 分）= 大幅達成**（計測 202.9 / 199.2 / 200.7・WU 212.4。**ばらつき 1.8%** と極小）
  - API「約 305 回」→ **実測 511 回（未達）**。差分 206 は **UPSERT 事前 GET 200 回（10,000 キー ÷ 50）+ 顧客読取 6** で全額説明可 — **目標値側の算出漏れ**。11 章の表は「約 505 回（事前 GET 含む）」へ改訂するか条件を注記すべき
- seed: exit 0・**2,282.5 秒（38.0 分）**（10,000+100,000）。CSV 5.99 MiB = **10 MiB 制限の 60%**（20 万件規模では CSV 分割が必要）。総売上 **500,050,550,000** 一致
- dry-run: SUCCESS・読取 100,000/206・update 10,000/100 チャンク・**プレフィックス外 0**・90.2 秒・RSS 263 MB（preview 照合 200 = dry-run も事前 GET 相当を消費）
- 集計 本実行 ×4: 全回 読取 110,000/書込 10,000/**API 511**・RSS ≈ 260 MB
  - 指標の注記（F-7 回答 §9 を受けて）: 本書の API 回数は **HTTP リクエスト数**。現行実装は bulkRequest 不使用のため、kintone の「1 日に実行できる API リクエスト数」（アプリ単位・スタンダード 10,000/日）の消費も同数。F-7（native upsert）適用後は両指標とも約 311 に減る。F-5（bulkRequest）は HTTP 往復数のみ減り、日次カウントは減らない
- 突合: SUCCESS・読取 340,036/API 691・166.9 秒・**RSS 279 MB（本検証全体の最大）**
- touch ×3: 100,000 件（1,000 チャンク）・API 1,292・**2,673.8 秒（44.6 分）**
- cleanup ×5: 110,000 件・API 1,459・525.4 秒 → precheck + MCP 残存 0
- **スケール線形性**: S 1.6s/9 → M 5.2s/17 → L 31.4s/107 → XL 200.7s/511（件数 5 倍で時間 6.4 倍・API 4.8 倍。非線形劣化なし）。RSS も 115 → 260 MB とほぼ比例、**11 章の「TEMP 20 万行 / 1 GB」想定と整合**
- **F-5（bulkRequest）起票条件を明確に充足**: 秒/チャンクが段階をまたいで安定（**UPDATE 2.67 / DELETE 0.48 = 約 5.6 倍**）→ チャンク数が所要時間を線形に決める。bulkRequest で書込 API 最大 1/20 → XL touch 44.6 分の大幅短縮が見込める。**XL 実測を添えて起票推奨**
- アプリ別 API（構成から確定）: 案件管理 ≈4,700 / 顧客管理 ≈1,700 / ログ ≈130。**1 アプリ 1 日 10,000 回の目安に対し最大 47%**。サイクル総 ≈ 6,500（見積 5,500 を 18% 超 — 集計 4 回 + dry-run preview 200 が主因）
- **429/5xx: 発生せず**（全 14 ジョブの error_message 空・リトライ記録なし）
- 観点 5: ログアプリ 14 レコード（108〜121）が read/written/api_calls/duration すべてコンソール実測と一致
- 実行後、config の limits は撤去してコミット状態へ復元

### XL（100,000 件）— 準備記録（2026-08-23 追加・上記で実機実施済み）
...
### 追記（2026-08-23 夕・Takashi 指示）: 単文 UPDATE 実験（L 20,000 件を 1 SQL で）

- **IN 列挙の単文は不可能と実測確定**: 会社名 2,000 社の IN（SQL 46KB）は kintone 入口の nginx が **HTTP 414** で拒否（書込ゼロ・1.4 秒 fail-fast）。dry-run 探索で境界を挟み込み: **250 社（IN 5.5KB）通過 / 270 社（6.0KB）= HTTP 431 / 300 社〜 = 414** — エンコード後 URL ≈ 8KB が崖。分割版の IN_BATCH=250 は偶然境界直下だった（会社名がより長い用途では要調整）。読取系はカーソル（POST ボディ）のためこの制限を受けず、**DML の候補取得（GET）だけが対象**
- **日付範囲条件の単文に転換（Takashi 提案）**: 当月範囲のみの短いクエリ + ASSERT 2 本（当月にテスト外 0 件・件数一致）で実データ保護
  - ケース①（上限未設定）: ASSERT の読取が既定 10,000 で明示エラー停止（書込ゼロ・4.7 秒）
  - ケース②（maxReadRows/maxTempRows 25,000）: **単文 20,000 件 UPDATE 完走 — 556.1 秒・書込 20,000・API 336・RSS 114 MB**（ログアプリ記録と完全一致・duration 555）
  - ASSERT 分（読取 40,000/API ≈80・≈50 秒）を除く UPDATE 実効 ≈ 2.53 秒/チャンク = **分割版 2.51 と一致 → 「文の分割は性能に無関係」を単文で直接証明**
- cleanup 22,000 件 → precheck 残存 0・config の limits は撤去済み
- 起票候補（条件つき）: F-6 = DML 候補取得の長大クエリを POST（X-HTTP-Method-Override 等）へ切替える対応 — 実務で大量 IN 指定が要件化した場合のみ（現行は分割 or 押し下げ可能条件で設計回避可能）

## 2026-08-25 追試: F-5 §6 の A/B 実測（bulkRequest 束ね効果）

- 条件: M 相当 2,000 件（案件管理・ルックアップ込み 20 フィールド超）の 詳細 UPDATE。A = 100 件 ×20 逐次 PUT / B = bulkRequest 1 回（20 サブリクエスト）。WU1 + 交互 3 セット
- 結果: A 中央値 49.8s（2.02〜2.84 s/チャンク = ベースライン 2.67 と整合）/ B 中央値 47.6s → **B/A = 96%。bulkRequest はほぼ効かない**（B3 のみ 11.8s の外れ値・再現なし）
- 結論: 書込時間はサーバ側処理が支配（エンジン F-5 回答 §2 の推測を実証）。F-5 保留に同意。seed 45.2s / cleanup 2,200 件 33 API / 残存 0 確認
- 副次知見: kintone の like は語単位マッチで `KSQL-FLOW-TEST-C` が `C200` に当たらない（測定はプレフィックスをクライアント側判定）

## 2026-08-25 追試: F-7 native upsert の縮小受入実測（M・エンジン v3.74.0）

- 構成: ランナー v0.4.0 相当（コード無変更・依存 ^3.74.0）+ エンジン v3.74.0（native upsert 既定 ON）
- M 集計ジョブ（読取 2,000 → UPSERT 200）: **API 17 → 14 回**・SUCCESS・書込 200・verify 全一致・cleanup 2,200 件 → 残存 0
- 内訳: 事前 GET 4 回（200 キー ÷ 50）が消滅、native 適格性判定のフォーム定義取得が +1 → 差引 -3（= 17 - 3 = 14 で完全一致）
- XL 換算: 事前 GET -200 + フォーム +1 = **511 → 312 回**の見込み（受入基準 1 の正式な XL 実測は未実施）
- ランナーのテスト: モックに native upsert セマンティクス（updateKey 照合・GAIA_IQ28・空文字拒否・operation）を実装し 160/160 green
