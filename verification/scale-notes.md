...
- 内訳: 事前 GET 4 回（200 キー ÷ 50）が消滅。+1 は【後日訂正】native 無関係 — ランナー v0.3.0 の deleted_count フォーム検出（ログアプリ GET form/fields・プロセス 1 回。基準値 17 はランナー v0.2.0 測定のため差が出た）
...
## 2026-08-25 追試: F-7 native upsert の正式受入実測（XL・エンジン v3.74.0）

- 構成: ksql-flow v0.4.0（依存 ^3.74.0）+ エンジン v3.74.0。XL seed 110,000 件（2,023.7 秒）・実行時 limits { maxReadRows: 120000, maxTempRows: 120000, batchTimeoutSec: 7200 }（終了後撤去済み）
- 集計ジョブ（UPSERT 10,000）WU + 3 回: **API 全 4 回で 312 回**（8/23 基準値 511 → -199 = 事前 GET -200 + ランナー v0.3.0 の deleted_count フォーム検出 +1【切り分け済み・native 無関係】）
- 読取件数 110,000 → **100,000**（事前 GET のキー照合 1 万件が読取カウントから消滅。M の 2,200 → 2,000 と整合）
- 所要時間 中央値 **197.8 秒**（185.6 / 197.8 / 200.0。基準値 200.7 比 -3 秒 — 時間はサーバ側処理支配で API 数減が効きにくい = F-5 A/B と同じ構図）・RSS ≈ 216〜262 MB
- 日次 API リクエスト数の消費も 511 → 312（bulkRequest 不使用のため HTTP = 日次カウント）
- cleanup 5 本で 110,000 件削除（API 1,464）→ precheck 残存 0

## 2026-08-25 追試: +1 の切り分け（M・HTTP パス記録）

- v3.74.0 native ON 14 / OFF 18 / v3.72.0 18 — **ON/OFF 差は事前 GET 4 本のみ・メタデータ系は 3 条件で完全一致**（form/fields 3 = 案件 + 顧客 + ログアプリ）
- **+1 の正体 = ランナー v0.3.0 の deleted_count フォーム検出（ログアプリへ GET /app/form/fields.json・プロセスあたり 1 回）**。8/23 基準値は v0.2.0 測定で、比較の両辺でランナー版が違っていた
- 教訓: 基準値と再測でランナー・エンジン両方の版数を固定して記録する（本書の各実測に構成行を必ず書く運用は正しかったが、差分解釈時に基準側の版を確認しなかった）
