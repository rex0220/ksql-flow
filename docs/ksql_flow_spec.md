...
* **保証する**: タイムアウト（`batchTimeoutSec`）以内に完了する実行について、同一 `job_key` の実行は同時に 1 つだけ成立する（既存ロックへの後発衝突は実測で確認済み。**バリア同期した同時 INSERT の競合は未実測**であり、重複禁止フィールドは公式に CAS として保証された API ではない）。
* **保証しない**: `batchTimeoutSec` 超過は**死亡確認ではない**。長い GC・VM サスペンド・ネットワーク分断では、生存中の保持者が stale と判定・回収され、復帰後に書き戻す窓が残る。この窓では新旧 2 実行が**異なる as-of** で同一キーに書くため、業務データの**古戻り・複数文の混在・誤削除**があり得る。旧保持者の終了更新は無条件のため、回収時の `TIMEOUT` が後から `SUCCESS` へ上書きされ得る点にも注意。
