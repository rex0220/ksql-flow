...
* キーに指定できるのは **「値の重複を禁止する」設定済みの文字列（1行）または数値フィールド**のみ。条件を満たさない場合は `ksql validate` がエラー（`KSQL1301`〜`KSQL1303`、Exit 1）で検出し、さらに**実行時にも書込 API 呼び出しの前に fail-closed で再検査**します（実行前に重複禁止設定を確認できない場合もエラー）。
  * 実装上の位置付け（エンジン v3.74.0 以降）: 素の UPSERT は既定で kintone の **native upsert**（`PUT /k/v1/records.json` の `updateKey` + `upsert: true`。1 リクエスト 100 件でキー存在判定を kintone 側に委ねる）で実行され、**従来の事前 GET（50 キー/回）は消えます**。適用条件を欠く文 — 空文字キーを含む・ソース全体でキーが重複する・`CHECK` / `APPLY` / `IMPORT` を伴う等 — は従来の **read-then-write**（GET で照合 → POST / PUT）へ自動フォールバックし、**結果はどちらの経路でも等価**です（挿入 / 更新の内訳はレスポンスの `records[].operation` で厳密に取得）。native は**更新だけで完結する UPSERT でも対象アプリの「レコード追加権限」を要求**します（kintone の仕様）。従来経路へ明示的に戻す場合はエンジンオプション `enableNativeUpsert: false`。本仕様のキー制約（重複禁止・単一・文字列 1 行 / 数値）は native の `updateKey` 要件とも一致しており、v3.72 系以前の read-then-write でも「非 unique キーでの upsert 先の曖昧さを塞ぐ方針上のチェック」として同一でした。
...
run-all の失敗時挙動は複数章にまたがるため、ここに要約します（詳細の正は各参照先。表の「失敗」は **FAILED 系 = `FAILED` / `ABORTED` / `TIMEOUT`** を指し、スキップ理由の表記は 8.2 の `SKIPPED (<理由>)` 契約と同一）。

| | 失敗したジョブ | 依存する後続（`depends_on` 宣言あり） | 独立した後続 | 終了コード |
| --- | --- | --- | --- | --- |
| **既定** | 残ステップ破棄・FAILED 系記録 | `SKIPPED (dependency: <ジョブ名>)` | **実行する** | 失敗の最重大コード（3 > 2 > 5 > 1） |
| `--stop-on-error` | 同上 | `SKIPPED (stop-on-error)` | `SKIPPED (stop-on-error)` | 同上 |
| `--continue-on-error` | 同上 | `SKIPPED (dependency: <ジョブ名>)` | 実行する | 成功と混在なら **4**（部分成功） |
...
* `job_key` は **`status = RUNNING` の間だけ値を保持**します。値はジョブレコードで `{profile}:{job_name}`、バッチレコードで `{profile}:__batch__`（いずれも日付を含まない）とし、「同一プロファイル・同一ジョブは常時 1 実行のみ」という意図をキー制約そのもので表現します（保証範囲は本節末尾）。
...
**ロックの保証範囲（本ロックはリース = 時限付き占有である）**:

kSQL Flow のロックは無条件の相互排他ではなく**リース**です。保証と非保証を明確に分けます。

* **目標とする性質**: タイムアウト（`batchTimeoutSec`）以内に完了する実行について、同一 `job_key` の実行は同時に 1 つだけ成立すること。根拠は kintone の重複禁止制約に依存する。**既存ロックへの後発衝突は実測で確認済み**だが、**バリア同期した同時 INSERT の競合は未実測**であり、重複禁止フィールドは公式に CAS として保証された API ではない — この依存は設計上の前提として明示する。
* **保証しない**: `batchTimeoutSec` 超過は**死亡確認ではない**。長い GC・VM サスペンド・ネットワーク分断では、生存中の保持者が stale と判定・回収され、復帰後に書き戻す窓が残る。この窓では、回収が**次回の新規定期実行**なら新旧 2 実行が**異なる as-of** で同一キーに書き、業務データの**古戻り・複数文の混在・誤削除**があり得る（`--resume` 経由の回収は同じ as-of を引き継ぐためリスクは縮小するが、入力が as-of で決定的・全書込が冪等でなければ同時実行の安全性は保証されない）。旧保持者の終了更新は無条件のため、回収時の `TIMEOUT` が後から `SUCCESS` へ上書きされ得る点にも注意。
* **この窓は「別の起動元からの回収」でのみ開く**: ローカルロックは同一ホストで PID が生存中の間、経過時間に関係なく回収しない（5.5-1）。したがって**同一ホスト・同一作業ディレクトリだけで起動する運用（単一 VPS の cron + ポーラー等）では、生存中の保持者から奪う経路が構造的に存在しない** — その間の次回実行は Exit 5 の静かなスキップになる（Exit 5 は通知を発しないため、生存ハングの検知は上記リマインダーが唯一の警報）。窓が開き得るのは、別ホスト（GitHub Actions・複数サーバー）または同一ホストでも別の作業ディレクトリの clone から実行した場合に限られる。複数経路で運用する場合はランブックの停止確認が特に重要になる。
* **前提となるジョブ設計**: 上記の残余リスクを受容できるのは「**全量再計算またはキー付き UPSERT・同一 as-of で決定的・冪等・誤適用後にバックフィル / 突合で復旧できる・DELETE を含まない（または削除対象を再構成できる）**」ジョブに限る。**DELETE を含む・外部状態に依存する・非冪等（加算・採番・通知等）・全量再計算を持たないジョブ**は、`batchTimeoutSec` を未設定または十分長く取り自動回収を実質無効化する・専用 profile に分離する・復旧は死亡確認後の手動 `unlock` のみとする、のいずれかで運用する。
* **運用前提**: 全起動ホストの NTP 同期（stale 判定はホスト間の時刻比較）・ログアプリにレコード単位のアクセス権を設定しない・全トークンが全レコードを閲覧可能・ログアプリのレコードを手編集しない。
* **必須の監視構成**: `TIMEOUT` への更新は「次回実行が回収した時点」で行われるため、**次回起動がなければハングは検知されない**。`status = RUNNING` が長時間続くレコードへの**リマインダーの条件通知**（`更新日時` 基準・発報時再評価）を必ず設定すること。

**stale 回収（TIMEOUT）検知時の復旧手順（ランブック）**:

「リランすれば直る」ではない。**旧保持者が停止したことを確認するまで、新しい実行を重ねない。**

なお**単一ホスト運用で保持者が生存ハングしている場合**（リマインダーで検知・TIMEOUT は未発生）は簡略化できる: ハングプロセスを停止すれば、次回の定期実行がローカル/分散の stale を自動回収して正常化する（TIMEOUT 通知が 1 通飛ぶのは正常な回収の記録）。以下のフル手順が必要になるのは、主に**別の起動元が既に回収してしまった後**の復旧である。

1. 対象 profile のスケジュール（cron・ポーラー・Actions）を一時停止する
2. ログの `host` / `batch_id` / `started_at` から旧保持者を特定する
3. 旧ホストでプロセスが停止済みかを確認する（生存していれば停止し、業務書込が止まったことを確認する）
4. その後にだけ `ksql unlock`（必要な場合）
5. 復旧を実行する: 同 as-of の `--resume`・バックフィル（`--as-of`）・または全量再計算
6. 突合で確認する
7. スケジュールを再開する

旧保持者の停止を**確認できない**場合: 新しいリランを重ねず、安全な静穏期間を置き、最後に全量再計算 + 突合で締める。
...
* **書き込みチャンク**: DML は **100 件 / リクエスト**で自動分割される（5.1、`bulkRequest` はエンジン未実装 — なお実測で束ねても所要時間はほぼ縮まないことを確認済み（B/A = 96%・サーバ側処理支配）。推定 API 消費の算出（7.2 / 10.2）もこのチャンク数に基づく。
* **native upsert（エンジン v3.74.0 以降）**: 素の UPSERT は既定で事前 GET なしの native 経路になり、書込は `ceil((挿入 + 更新) / 100)` リクエスト（従来: 事前 GET `ceil(キー数 / 50)` + `ceil(挿入 / 100) + ceil(更新 / 100)`）。dry-run の `estimatedWrites` もエンジン側でこの式に追随済み。適用条件と fallback は 3.4 参照。
...
* いずれの場合も、作成後に `ksql validate --check-logapp` でフィールド定義の過不足（`job_key` の重複禁止、`record_type` の BATCH/JOB、`status` の選択肢集合を含む）を検査できる。既存アプリを流用する場合も同様。standalone 互換のため、orchestrator/kSQL-FlowNet 用の相関 5 フィールドと `status` の `CANCELLED` は未追加でも情報表示に留め、それ以外の不足・過剰は不備とする。
