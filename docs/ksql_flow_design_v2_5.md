...
* `depends_on` が宣言されたジョブは、依存元が `FAILED` / `ABORTED` の場合のみ `SKIPPED` になります。**依存関係のない独立ジョブは、他ジョブの失敗に巻き込まれません**（v1 の「ファイル順で後続を一律スキップ」を改め、宣言された依存のみを停止伝播の対象とします）。
...
| **API 一時エラー** | HTTP 429 / 500 / 502 / 503 / 520 / タイムアウト | 指数バックオフ + Retry-After で自動再試行。上限超過時は中断 | `FAILED` (Exit 3) |
...
| `last_written_key` | 最終書込キー | 文字列 | 部分適用時の影響範囲判断用チェックポイント（→ 5.1） |
...
* **アラート通知**: kintone 標準機能（「`record_type = BATCH` かつ `status = FAILED` の時に管理者へメール通知」）で完結。`NO_DATA` は通知対象にならないため、誤発報が起きない。
