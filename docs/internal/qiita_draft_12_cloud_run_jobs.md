...
- **Scheduler の実行要求は予定時刻 +0.96〜0.99 秒（2 回の実測）** — 測定点は違えど cron 運用と遜色ない頭出しで、分単位の定刻運用に十分
