...
- 遅延の実測: 6:07 設定に対し**実際の起動は 6:37 — 30 分遅れ**でした（後述の実機検証）。分単位の正確さは最初から期待しないでください
- cron の**変更は発火時刻の直前に行わない** — 発火 19 分前の変更でその回が実行されない事象を実機で確認しています（反映に時間がかかることがある）
...
| schedule 自然発火（6:07 設定） | 実際の起動は **6:37:37 JST（+30.5 分の遅延）** — 「ベストエフォート」の生きた実例。as-of 基準の集計なので結果は同じ（<!-- TODO: 有効化後の本実行版を翌朝再確認 -->） |
| schedule の変更反映 | 発火予定の **19 分前に cron を変更したところ、その回は実行されなかった**（変更の反映には時間がかかることがある）— 発火時刻の変更は余裕をもって |
