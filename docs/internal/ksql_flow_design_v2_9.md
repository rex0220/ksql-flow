...
**Version 2.9（2026-08-22 改訂）**

> v2.9 での変更点（fix5 / 公開契約 baseline）: CLI と config を未知入力に対して fail-closed 化し、config JSON Schema、dry-run JSON `formatVersion: 1`、JSONL 共通 envelope、profile 別 state path / 警告 fallback を v0.1 公開契約として確定した。ログアプリ選択肢検査、Node 18 / pack CI、unlock の全 RUNNING blast radius も公開ゲートとして明文化した。clientCert / proxy と validate の推定表示は v0.1 非対応へ scope を一意化した。裁定 Q1〜Q7 は変更しない。
...
5. `--force-unlock` または `ksql-flow unlock` で、ハングした前回セッションのロックを明示的に解除できます。**解除対象は batchId / jobKey 単位ではなく、同一プロファイルの全 RUNNING レコード**です。解除前に jobKey・開始時刻・対象件数を一覧表示し、解除の事実もログに記録します。有効な別ジョブを巻き込む blast radius があるため、一覧を確認して使用します。batchId / jobKey 指定解除は v0.1 では実装せず、POST 課題とします。
...
* **CI での静的検証**: PR ごとに `ksql validate-all ./jobs/ --profile stg` を実行し、構文・論理名・updateKey 制約を機械チェックする。ランナーの `validate` は公式 API `/flow` の `validateScript(source, { client })` を用いるためスキーマ依存検証を含むフル検証となる。一方 MCP の `ksql_validate` はネットワーク 0 の契約で updateKey 等の検証を含まないため、**AI 生成時の一次チェックは MCP、確定チェックはランナーの validate** という二段構えになる（申し送り 4）。推定 API 消費・推定所要時間は v0.1 の validate では提供せず、`--dry-run` の EXPLAIN / preview で確認する。
...
| resume の正が二重（ログアプリ / state） | 6.2（ログアプリを正、`.ksql/state-<profile>.json` はフォールバック） |
...
### v2.8 → v2.9（fix5 / PRE-1〜PRE-6 公開ゲート）

| 変更 | 反映箇所 |
| --- | --- |
| CLI allowlist、未知・不適用・競合・余分 positional の fail-closed、dry-run 専用 flag | 10.1 |
| clientCert / proxy と validate 推定表示を v0.1 非対応へ一意化 | 7.2 / 9.1 / 10.1 / 14 章 |
| config JSON Schema / unknown-key error、JSON formatVersion、JSONL envelope、profile 別 state と警告 fallback | 6.2 / 8.3 / 9 章 / 10.2 |
| ログアプリ選択肢検査と template 契約突合 | 8.2 / 付録 B |
| Node 18 matrix と npm pack / install / help smoke | 14 / 15 章 |
| unlock / force-unlock の同一 profile 全 RUNNING blast radius と対象事前表示 | 5.5 / 10.1 |
