...
ランナーは終了時にレコードの status を書き換えるので、失敗の瞬間に kintone がポータル・メール・モバイルアプリへ通知します。宛先の管理も kintone 側（ユーザー/組織/グループ）なので、担当が変わっても config を触りません。Webhook（Slack 等）は「ログアプリに書けないほどの失敗でも飛ぶ」「heartbeat が打てる」という一段深い保険として、必要な環境だけ追加すれば十分です。
