...
FAILED / ABORTED / TIMEOUT で文を分けないのは、初動が全部「error 欄を見る」で同じだからです。「どのジョブか」はレコードタイトルに任せます — アプリ設定（その他の設定 → レコードのタイトル）を **`スクリプト名`** にしておくと、単発ジョブの通知一覧にジョブ名がそのまま並びます（BATCH のタイトルは実行対象ディレクトリ `.\jobs` になりますが、バッチ失敗であることは固定文で分かるので実害はありません）。
