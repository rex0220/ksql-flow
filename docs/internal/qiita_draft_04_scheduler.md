...
- **「実行に失敗したとき再起動する」は設定しない**（理由は後述）
- **`-StartWhenAvailable`** を入れてあるので、Windows Update の夜間再起動などで 6:00 に電源が入っていなくても、**起動後に追いかけて実行**されます（後述の「起動しなかった事故」のうち、この型だけは設定で拾えます）
- 上の `-LogonType Interactive` は**ログオン中しか動かない検証向けの設定**です — ログオフ中も動かす本番運用では、専用アカウントの「ユーザーがログオンしているかどうかにかかわらず実行」に切り替えます。ハマりどころは認証で、専用アカウント + パスワード保存（`Register-ScheduledTask` の `-User`/`-Password`）か、パスワード管理が不要の **gMSA** を使います。SYSTEM アカウントで動かす手もありますが、`.env` を読めるフォルダー権限（NTFS を絞った場合）やプロキシ設定がユーザーと異なる点に注意してください
...
通知メッセージは固定文しか書けない（ジョブ名やエラー内容は差し込めない）ので、**「深刻度 + 初動 1 つ」だけを書く**のがコツです。例:

- 条件 1: `【要対応】kSQL Flow バッチ失敗 — このレコードの error 欄を確認し、原因対処後に --resume で再実行`
- 条件 2: `【要対応】kSQL Flow ジョブ失敗 — このレコードの error 欄を確認（対処後の再実行は run で可）`

実際の設定画面です（2 条件・通知内容・宛先のジョブ管理グループ）:

![2026-08-24_19h58_03.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/100572/29740328-5599-46ad-883f-fcb12b406947.png)

FAILED / ABORTED / TIMEOUT で文を分けないのは、初動が全部「error 欄を見る」で同じだからです。「どのジョブか」はレコードタイトルに任せます — **同梱テンプレートから作ったアプリはレコードのタイトル = `スクリプト名` が設定済み**で、単発ジョブの通知一覧にジョブ名がそのまま並びます（既存アプリは「その他の設定 → レコードのタイトル」で変更。BATCH のタイトルは実行対象ディレクトリ `.\jobs` になりますが、バッチ失敗であることは固定文で分かるので実害はありません）。
...
メール通知には、システム管理の「利用する機能の選択 → 通知のメール送信」が有効であることが前提です。見落としやすいのが **「REST API の通知をメールで送信」** — ランナーのレコード書込は REST API 経由なので、ここが OFF だとポータルには通知が出てもメールは飛びません:

![2026-08-24_20h00_17.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/100572/b4bbf735-9e88-4d09-bead-85250f5354d1.png)

設定後の動作確認には、テンプレート同梱の通知テストジョブ（`dev/notify_test/` — 意図的な ASSERT 失敗・業務アプリへの書込ゼロ）を単発 run と `run-all ./dev/notify_test` の 2 通りで流します。この 2 条件 + 固定文 + タイトル = スクリプト名の構成は**実機で確認済み**で、通知メールは件名に「タイトル - 固定文」が入るため、件名だけで「どのジョブが失敗し、何をすべきか」まで読めます。

実際に飛んだ通知（ポータル）。固定文とレコードタイトルが並び、この 2 行だけで「バッチかジョブか・どのジョブか・何をすべきか」まで読めます:

![2026-08-24_20h07_23.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/100572/1ea2472e-8fde-45ac-8bf2-29bdd21072da.png)

Webhook は「ログアプリに書けないほどの失敗でも飛ぶ」「heartbeat が打てる」という一段深い保険として、必要な環境だけ追加すれば十分です。失敗通知の payload は Slack の Incoming Webhook がそのまま受け取れる形（`text` フィールド同梱）です。heartbeat は「毎朝飛んでくるはずの信号の不在を検知する」ためのものなので、宛先は Slack ではなく healthchecks.io のような不在検知サービスが向いています。
...
最後にもう一度正直に。ランナーがどれだけ装備を持っても、**スケジューラがそもそも起動しなかった事故**（サーバー停止・タスク無効化）はランナーには観測できません。Windows Update の再起動で定刻をまたいだケースだけは `-StartWhenAvailable` が起動後に追いかけてくれますが（タスク登録の節参照）、サーバーが上がってこなければそれも動きません。手当ては heartbeat です — 成功時にも Webhook を打つ設定にしておき、「**毎朝来るはずの通知が来ない**」ことを検知の頼りにします。通知チャネル側の仕組み（Slack のリマインダーや外部監視）で「不在」を拾えるならなお良し。ここを自動化しきれないことが、タスクスケジューラ運用の受け入れるべき限界です。
