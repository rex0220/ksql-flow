...
最初の設計は「リモート起動アダプター」でした。ポーラー（どこかで常駐）が Cloud Run Jobs の API を叩いてバッチジョブを起動し、完了を待って、結果の Exit Code を Cloud Logging のログから回収する — 実装は一通り動き、E2E テストも通りました。
...
最後の行には理由があります。kintone の日時フィールドは**分単位に切り捨て**られる（[#10](https://qiita.com/rex0220/items/b841921afe86083f14a0) の実測）ため、同じ分に始まった 2 つのバッチはどちらが新しいか判定できません。判定できないときに推測で動かない — 不確定は人間に返す、[#10](https://qiita.com/rex0220/items/b841921afe86083f14a0) からの原則です。
...
[#12](URL_TBD_12) で「従量制は回数より 1 回の処理時間が効く」と書きました。ポーラーはその**例外**です。
...
- **時間帯を絞る** — リラン指示が出るのは実質「朝のバッチ失敗後」。Scheduler の cron を `*/5 6-8 * * *`（朝 6〜9 時だけ）にすれば月 1,080 分で無料枠内。時間帯外のチェックは消えず、翌朝のウィンドウで拾われます（要求の有効期限だけ時間帯に合わせて調整）
- **間隔を広げる** — 15 分間隔なら月 2,880 分で枠内。リラン応答は最大 15 分待ちに
- **ポーラーだけ VPS に残す** — バッチはサーバーレス・ポーラーは既存 VPS という分担も、ホストラベルでの担当分離により安全に共存できます

即応性・費用・構成の単純さの三択で、正解は運用次第です。私たちの検証環境は「平常時は停止・検証時だけ再開」で運用しています。
...
- 高頻度ポーラーは**最低 1 分課金**に食われる — 時間帯限定・間隔調整・VPS 併用から運用に合わせて選ぶ
