...
**1 つ注意**: この UPSERT は「今回の集計に出てきた会社」だけを更新します。**今月の案件が 0 件になった会社の前月値は、そのまま残ります**（月が変わった直後も同じ）。洗い替えたい場合は、集計対象から消えた会社をゼロクリアする DELETE / UPDATE を 1 文足してください — 「書いた値を消す責任は書いた側にある」という、冪等設計でいちばん見落とされる点です。
