...
kintone でアプリ間集計や定期データ更新（月次売上をマスタへ反映する、ような処理）をやろうとすると、だいたい二択になります。

1. **クラウド型 ETL サービス** — 高機能だけれど、ベンダーのサーバーに認証情報とデータを預けることになり、セキュリティ監査のコストが重い
2. **自作の JavaScript / REST API スクリプト** — 自由だけれど、リトライ・排他制御・失敗時のリランを毎回自前で書くことになり、確実に属人化する

kSQL Flow はこの二択の間を狙ったツールです。**SQL スクリプト 1 本でパイプラインを定義**し、実行・排他・リラン・実行ログ・リトライ・通知といった「バッチの面倒な部分」はランナーが引き受けます。実行場所は自分の環境（ローカル / 社内サーバー / GitHub Actions / Docker）で、**通信先は設定した kintone と通知 Webhook だけ**。テレメトリの類は一切ありません（BYOC: Bring Your Own Cloud）。
