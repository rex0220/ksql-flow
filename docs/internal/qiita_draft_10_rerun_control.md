<!--
title: 【kSQL Flow #10】スマホのチェック 1 つで VPS のバッチをリランする — ポートを 1 つも開けずに
tags:
  - kintone
  - SQL
  - 運用
...
本記事はその回収編です。結論から書くと、**kintone の失敗レコードにあるチェックボックスへ印を 1 つ付けるだけ**で、5 分以内に VPS 側が `--resume`（冪等リラン）を実行し、結果が同じレコードに返ってくるようにしました。SSH 鍵は不要。スマホのブラウザ（kintone アプリ）で完結します。そして **VPS のポートは 1 つも開けていません**。
...
| 案 | 出先操作 | 書込トークンの露出面 | サーバーのポート開放 | 判定 |
| --- | --- | --- | --- | --- |
...
flowchart TB
    ...
    subgraph vps["VPS（#9 の構成のまま・ポート開放なし）"]
...
**exactly-once は保証しない。保証しなくてよい。** リランは冪等（[#7](https://qiita.com/rex0220/items/39821af2a79b88de0ed2)）で、同じ指示が 2 回実行されても最終状態は同じ。万一の同時実行もランナーの分散ロックが Exit 5 で止めます。だからキューに必要なのは at-least-once — 「いつか必ず 1 回以上実行される」ことだけで、重い状態機械が要りません。**下の層（冪等リラン + 分散ロック）が強いほど、上の層は単純にできる**。この仕組みが VPS 側スクリプト 1 本で済んだ理由です。
...
これが**ほぼ毎回空振りします**。kintone の DATETIME フィールドは**分単位に切り捨てて保存される**ため、claim と同じ分に始まった resume の `started_at` は `12:37:00Z` になり、「12:37:45 より後」を満たせません。基準時刻を分頭に floor して `>=` で比較する形に直しました。ローカルテストのモックは形式まで再現しないので、**実機ドリルで初めて出る**類のバグです（`Date#toISOString()` が返すミリ秒 `.789Z` も kintone は受け付けないので、そちらも要注意です）。
...
- **ポートは 1 つも開けない**。VPS からの outbound HTTPS ポーリングのみで、#9 のセキュリティ構成は無変更
...
- 保証は下の層に寄せる — 冪等リラン + 分散ロックがあるから、上のキューは at-least-once + 楽観ロックの単純な作りで足りる
