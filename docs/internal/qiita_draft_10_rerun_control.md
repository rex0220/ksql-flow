...
flowchart TB
    SSH["SSH で入って手動実行"]
    KIN["kintone のチェックボックスに印<br>→ ポーラーが検知"]
    CMD["./run_batch.sh --resume<br>同一のコマンド = まったく同じ動作"]
    SSH --> CMD
    KIN --> CMD
```

**SSH の手動経路はそのまま残ります**（ランナーも起動スクリプトも無変更なので、当然に）。両方やってしまっても壊れません — 衝突すれば後から走った側が分散ロックの Exit 5 で待ち、手動で先に復旧していれば、後から動いたポーラーの resume は「再実行が必要なジョブなし」の Exit 0 で静かに終わります（どちらも実機で確認済み）。差が出るのは監査面だけで、手動リランは `rerun_` 欄に証跡を残しません（実行の記録は resume が作る新しい BATCH レコードに残ります）。
...
実際のリラン指示は、こうです。**PC 版なら**、一覧のインライン編集を有効にしておけば、レコードを開かずに一覧から直接チェックできます（下のスクリーンショット）。**スマホのモバイル版では**、対象レコードを開いて［編集］からチェックを入れます — どちらも操作はチェック 1 つで同じです:

![リラン要求のチェック操作（一覧から直接）](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/100572/54b24b5a-eaae-4a2a-bde6-7cc5f0f3a244.png)

これが操作のすべてです。ちなみにこの画面をよく見ると、上の JOB 行にも同じチェック欄が見えています — 後述の罠③（人間は JOB 行をクリックする）は、まさにこの並びから生まれました。

**指示を間違えたらどうなるか**も先に書いておきます。原則は「間違えても壊れない。黙って無視もしない — 理由が返る」です。

| 指示ミス | 挙動 |
| --- | --- |
| リラン成功後にもう一度チェック | 正当な再要求として実行。直近が成功済みなら「再実行が必要なジョブなし」の **no-op（`SUCCESS`）に収束** — 冪等なので何度やっても安全 |
| 成功したバッチにチェック | 実行せず **`CANCELED` + チェック解除** + 「失敗した実行の記録を選び直してください」 |
| 子ジョブ（JOB 行）にチェック | 実行せず **`CANCELED` + チェック解除** + 「BATCH（親レコード）にチェックしてください」— 親をチェックすれば、その失敗ジョブは resume の選抜で自動的に再実行されます |
...
リラン完了後のレコードがこちらです。**チェックは自動で外れ、状態・要求者・結果・実行バッチ ID が同じレコードに埋まっています** — 「結果が同じレコードに返る」はこの画面のことです:

![リラン完了後のレコード（SUCCESS・要求者・実行バッチ ID が記録される）](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/100572/54286395-edb9-4209-aa66-715d324b2366.png)
...
- ポーラー停止の検知は**リマインダー通知**（10 分間隔の巡回・発報時再評価・1 回きり）で、**正常時の誤報なしに組めた**（今回の構成での実測）。条件通知で作ると毎回誤報する
