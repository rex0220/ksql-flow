...
ポイントは **`job_key` に日付を含めない**ことです。「同一プロファイル・同一ジョブの並行実行を 1 つに絞る」という排他の意味論を、キーの設計そのもので表現しています。実行記録とロックが**同じレコード**なので、ロック専用のアプリもテーブルも要りません — [#4](https://qiita.com/rex0220/items/d0a66c133edd42ff91c4) で作ったログアプリが、実行履歴・resume の正・**ロックサーバー**の三役を兼ねます。
...
| 層 | 実体 | 守備範囲 | stale 判定 |
| --- | --- | --- | --- |
| ローカルロック | `.ksql/lock-<profile>.json`（作業ディレクトリ直下・Git 管理外。`O_EXCL` で作成・PID/ホスト/開始時刻を記録） | **同一作業ディレクトリ**（同じ `.ksql` を共有する実行）の多重起動 | プロセス生存確認 + タイムアウト |
| 分散ロック | ログアプリの `job_key`（重複禁止）への INSERT | **マシンをまたぐ**多重起動 | `started_at` + `batchTimeoutSec` 超過 |

ローカルロックは API を 1 回も呼ばずに衝突を落とせる高速な門番で（守備範囲は cwd 単位 — 同じサーバーでも別 clone は共有しません）、プロセスの生存確認ができるぶん stale 判定が正確です。ただし**生存確認は同一ホスト内でしか効きません** — 別マシンのプロセスが生きているかはファイルからは分からない。だからマシンをまたぐ判定は分散ロック側の時間ベース規則（後述）に委ねます。役割が違うので、二層は冗長ではありません。

### このロックは「リース」— 保証範囲を先に正直に

正確に言うと、これは無条件の相互排他ではなく**リース（時限付き占有）** です。`batchTimeoutSec` の超過は**死亡確認ではない** — 長い GC・VM のサスペンド・ネットワーク分断では、**生きている保持者が stale と判定・回収され、復帰後に書き戻す窓**が残ります。この窓で怖いのは as-of です — 回収したのが**次回の定期実行なら新しい as-of** を固定するため、新旧 2 つの実行が異なる as-of で同一キーに書き、業務データの古戻りや複数文の混在があり得ます（`--resume` 経由なら同じ as-of を引き継ぐぶんリスクは縮みますが、入力が決定的・書込が冪等でなければ同時実行の安全は保証されません）。

面白いのは、**この窓が「別の起動元からの回収」でしか開かない**ことです。ローカルロックは同一ホストで PID が生きている限り、経過時間に関係なく奪いません — つまり**同一ホスト・同一作業ディレクトリだけから起動し、ロックを手動解除（`unlock` / ロックファイル削除）しない運用なら、生きているプロセスから自動的に奪う経路は存在しない**（次回実行は Exit 5 で静かにスキップし続けるだけ）。窓が現実になるのは、Actions や別サーバー、同じサーバーでも別 clone など**別の起動元**を足したときです。一見不便な「生存中は回収しない」というローカルロックの仕様が、実は窓への防壁になっています。

だから目標とする性質はこう限定されます: **タイムアウト内に完了する実行について、同一キーの実行は同時に 1 つ**（kintone の重複禁止制約が並行時にも維持されることを前提に）。窓の残余リスクは、ジョブを「全量再計算・キー付き UPSERT・冪等」に寄せる設計（[#7](https://qiita.com/rex0220/items/39821af2a79b88de0ed2)）と、**復旧手順の明確化**で管理します — TIMEOUT を見たら「リランすれば直る」ではなく、**スケジュールを止め、旧プロセスの停止を確認してから**リラン・バックフィル・突合で復旧します（手順の全文は[公開仕様書 §5.5](https://github.com/rex0220/ksql-flow/blob/main/docs/ksql_flow_spec.md)）。機構で窓を消しにいくより、単純なリースを保ちリカバリーを明確にする — 検討の末この設計に落としました。
...
```mermaid
flowchart TB
    ACQ["実行開始<br>job_key = prod:job名 で<br>RUNNING を INSERT"]
    OK["INSERT 成功 = ロック取得"]
    NG["400 = 重複の可能性<br>再検索で RUNNING を確認 → Exit 5"]
    RUN["ジョブ実行"]
    REL["終了時（成功でも失敗でも）<br>同一レコードを UPDATE:<br>job_key をクリア →<br>job_key_done へ退避"]
    STALE["クラッシュで RUNNING が残った場合<br>次回実行が started_at +<br>batchTimeoutSec 超過を検知<br>→ TIMEOUT へ更新して自動回収（警告つき）"]

    ACQ --> OK --> RUN --> REL
    ACQ --> NG
    RUN -.->|"死亡・長時間停止・分断"| STALE
...
- **クラッシュで `RUNNING` が残ったら**（= stale ロック）、次回の実行が `started_at + batchTimeoutSec` の超過を検知して**警告つきで自動回収**します（status を `TIMEOUT` に更新 — FAILED 系なので[条件通知](https://qiita.com/rex0220/items/d0a66c133edd42ff91c4)も飛びます）。ただし **TIMEOUT になるのは次回実行が回収した時点**です — 次回の起動が無ければ `RUNNING` のまま誰にも知られません。だから「`RUNNING` が長時間更新されない」ことを検知する**リマインダー通知を必須の監視構成**にしています（[仕様 §5.5](https://github.com/rex0220/ksql-flow/blob/main/docs/ksql_flow_spec.md)）。人手の解除は `ksql-flow unlock` — 同一プロファイルの**全 RUNNING レコード**を対象に、jobKey・開始時刻の一覧を表示してから `job_key` をクリア（`job_key_done` へ退避・status は `TIMEOUT`）します。解除の事実もログに残ります
...
ロック設計への含意は明確です。もし「`like` で RUNNING の存在を確認してから実行を決める」**check-then-act 型の素朴な実装**だったら — 今回の測定条件に当てはめると、**8 回中 6 回の見落としに相当**します。kSQL Flow が無事なのは、最終判定を `like` 照合ではなく**重複禁止フィールドへの INSERT** に置いているからです（確認検索を取りこぼしても、INSERT が重複制約で拒否され、確認に失敗すれば fail-closed で止まる）。それでも教訓は残ります: **ロック・排他・直後リードの照合は、必ず完全一致（`=` / `IN`）か `$id` で行い、全文検索を判定経路に置かない** — 以後、規約になりました。
...
kintone には **「重複禁止 INSERT（create-if-absent 相当）」と「revision 付き PUT（楽観的競合検出）」という 2 つの競合制御の道具**があります。**全量再計算・決定的・冪等で、突合やバックフィルで復旧できるジョブ**（[#7](https://qiita.com/rex0220/items/39821af2a79b88de0ed2) の定石に沿ったジョブ）なら、排他と競合検出はこの 2 つの道具で外部ミドルウェアなしに運用できる — 連載を通じて Redis も RDB も追加しなかった理由です。

## まとめ

- 多重起動はバッチ運用の典型事故で、起動経路が増えるほど（cron・手動・Actions・ポーラー）ロックの価値が上がる。ただしこのロックは**リース**であり、目標とする排他範囲は**タイムアウト内まで**。超過後の残余リスクを受容できるのは、**入力が同一 as-of で決定的・書込が冪等で、突合やバックフィルにより復旧可能なジョブ**に限る — 残余リスクは復旧手順で管理する
- kintone の**重複禁止フィールドへの INSERT を create-if-absent 相当の排他に使う** — ログアプリがそのままロックサーバーになり、実行履歴・resume の正と三役を兼ねる（既存ロックへの後発衝突は実測済み・同時 INSERT 競合は未実測で CAS の公式保証もない）
...
- 対策は検査より**生成規約**が上流で安く効く — 「更新系の WHERE に KLIKE を書かない」「同一バッチ実行内で書込アプリを KLIKE で読まない」の 2 行
- kintone の 2 つの競合制御 — **重複禁止 INSERT**（排他）と **revision 付き PUT**（楽観）— で、**冪等・決定的・復旧可能なジョブ**なら排他・競合検出を外部ミドルウェアなしに組める
...
11. **#11（本記事）**: [kintone がロックサーバーになる — 重複禁止フィールドで作る分散ロックと、検索索引ラグの実測](https://qiita.com/rex0220/items/44cf9f6d23c264d14dca)
