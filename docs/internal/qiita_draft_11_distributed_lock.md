...
- 前回: [【kSQL Flow #10】スマホのチェック 1 つで VPS のバッチをリランする — 新しいポートを 1 つも開けずに](https://qiita.com/rex0220/items/dd8ec668b8966e346d48)

この連載には **Exit 5（多重起動検知）** が何度も登場してきました。[#7](https://qiita.com/rex0220/items/39821af2a79b88de0ed2) では失敗分類の 1 つとして、[#10](https://qiita.com/rex0220/items/dd8ec668b8966e346d48) ではポーラーと通常バッチの衝突を安全に「待ち」へ変える仕組みとして。その土台が**分散ロック**です。
...
「そんなもので本当に競合に耐えるのか」を、連載で積んだ実測（強制切断・2 プロセス同時起動・stale 回収）で答えます。後半では、この設計の裏で見つかった **kintone 全文検索の索引反映ラグ**の実測（エンジン開発側の観測記録・引用許諾済み）も紹介します — 「ロックの照合に `like` を使ってはいけない」の実データです。

## なぜロックが要るのか — 多重起動はバッチ最頻の事故

バッチ運用でいちばん多い事故は、派手な障害ではなく**多重起動**です。
...
kintone には行ロックもトランザクションもありません（[#7](https://qiita.com/rex0220/items/39821af2a79b88de0ed2)）。では何を使うか。

## 鍵は「重複禁止フィールドへの INSERT」

kintone のフィールド設定には「**値の重複を禁止する**」があります。重複する値を書き込もうとすると、API は **400 エラー**を返します。この検査は kintone 側で行われるため、複数のクライアントが同時に同じ値を INSERT しても、**成功するのは必ず 1 件だけ**です。

つまり重複禁止フィールドへの INSERT は、分散環境における**原子的な check-and-set** として使えます。これがロックの正体です。
...
- 400 が返れば**誰かが実行中** — 何もせず Exit 5（`LOCKED`）で終了
...
| 層 | 実体 | 守備範囲 | stale 判定 |
| --- | --- | --- | --- |
| ローカルロック | `.ksql/lock-<profile>.json`（`O_EXCL` で作成・PID/ホスト/開始時刻を記録） | **同一マシン内**の多重起動 | プロセス生存確認 + タイムアウト |
...
- **`job_key` は `RUNNING` の間だけ値を持ちます。** 終了時（`SUCCESS` でも `FAILED` でも）に同一レコードの UPDATE で値をクリアし、**`job_key_done`（重複禁止なしの通常フィールド）へ退避**します。kintone の重複禁止は**空値の重複を許す**ので、クリアすれば翌日の INSERT は通ります — 前回が失敗で終わっていても、リランや翌日の定期実行が重複制約を踏むことはありません
- **クラッシュで `RUNNING` が残ったら**（= stale ロック）、次回の実行が `started_at + batchTimeoutSec` の超過を検知して**警告つきで自動回収**します（status を `TIMEOUT` に更新 — FAILED 系なので[条件通知](https://qiita.com/rex0220/items/d0a66c133edd42ff91c4)も飛びます）。人手の解除は `ksql-flow unlock`
...
「重複禁止 INSERT は原子的」は kintone の仕様から導ける推論ですが、連載の流儀は実測です。3 つの実験を重ねてきました。
...
**③ 2 プロセスの同時起動（[#10 の実機ドリル](https://qiita.com/rex0220/items/dd8ec668b8966e346d48)）** — ポーラーを 2 プロセス同時に走らせ、同じ失敗レコードを取り合わせる実験。結果は毎回「**確保も実行もちょうど 1 回**」。さらに通常バッチの実行中にリランを起動する実験では、後発が必ず Exit 5 で待ちに回りました。**4 つの起動経路が共存できるのは、この 1 点がすべての土台**です。
...
ロック設計への含意は明確です。**「いま RUNNING のレコードがあるか」を `like` で照合していたら、8 回中 6 回すり抜けます** — 排他が確率的に破れる。ロック・排他・直後リードの照合は、必ず完全一致（`=` / `IN`）か `$id` で行う。kSQL Flow のロック照合が最初から `=` だったのは幸運もありますが、以後は規約になりました。
...
この問題への kSQL Flow 側の対応は、ランナーの検査でもエンジンの警告でもなく、**ジョブを書くときの規約 1 行**でした。

> 書込は最後に置く。書込直後の全文検索（KLIKE）をジョブ内に置かない。

全 SQL 資産を機械検索して該当ゼロを確認したうえで、AI にジョブを書かせる際の規約（[#2](https://qiita.com/rex0220/items/3a1213a596a8c49b67aa) の CLAUDE.md）へ 1 行足しただけです。**検査より生成規約のほうが上流で安く効く** — 事後に検出する仕組みを作るより、そもそも生成されない形にするほうが、コードも保守も増えません。

## 排他ロックと楽観ロック — kintone の 2 つの原子性

まとめとして、連載で使った 2 種類のロックを並べます。どちらも kintone の標準機能だけで成立しています。

| | 排他ロック（本記事） | 楽観ロック（[#10](https://qiita.com/rex0220/items/dd8ec668b8966e346d48)） |
| --- | --- | --- |
| 目的 | **実行を 1 つに絞る**（多重起動防止） | **更新の競合を検出する**（後勝ち上書き防止） |
| kintone 側の原子性 | 重複禁止フィールドへの **INSERT**（成功は必ず 1 件） | **revision 条件付き PUT**（版が違えば拒否） |
...
kintone にトランザクションはありませんが、**「重複禁止 INSERT」と「revision 付き PUT」という 2 つの原子的操作**はあります。バッチ基盤に必要な排他と競合検出は、この 2 つと冪等設計（[#7](https://qiita.com/rex0220/items/39821af2a79b88de0ed2)）の組み合わせで足りる — 連載を通じて Redis も RDB も追加しなかった理由です。

## まとめ

- 多重起動はバッチ最頻の事故で、起動経路が増えるほど（cron・手動・Actions・ポーラー）ロックの価値が上がる
- kintone の**重複禁止フィールドへの INSERT は原子的な check-and-set** — ログアプリがそのままロックサーバーになり、実行履歴・resume の正と三役を兼ねる
- 設計の要は解放側: **`job_key` は RUNNING の間だけ**・終了時に `job_key_done` へ退避（空値の重複は許されるため翌日の実行が通る）・stale はタイムアウトで次回実行が自動回収・確立できなければ **fail-closed**
- 競合の実測: 強制切断からの回収収束（#3）・stale 回収と通知（#7）・**2 プロセス同時起動で確保は必ず 1 回**（#10）
...
- kintone の 2 つの原子性 — **重複禁止 INSERT**（排他）と **revision 付き PUT**（楽観）— で、バッチ基盤の排他・競合検出は外部ミドルウェアなしに組める
...
10. **#10**: スマホのチェック 1 つで VPS のバッチをリランする — 新しいポートを 1 つも開けずに（公開後にリンク差し替え）
