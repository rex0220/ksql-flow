...
第 1 段階の運用実績を確認後、スクリプト、秘密値を含まない設定例、cron 例、ログアプリへの 10 フィールド追加手順を `ksql-flow-template` へ移す。kSQL Flow npm パッケージへの公式サブコマンド化、テンプレート ZIP への 10 フィールド反映、公開 Schema の追加は第 2 段階で別途扱う。
