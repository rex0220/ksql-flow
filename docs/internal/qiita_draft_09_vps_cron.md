...
[#2](https://qiita.com/rex0220/items/3a1213a596a8c49b67aa) では「AI がジョブ SQL を書き、人間が dry-run をレビューして本実行する」という分担でした。今回はその**インフラ版**です。そして分担の境界は #2 と同じ考え方になりました — **秘密を「決める・発行する・渡す」のが人間、与えられた権限の範囲で操作するのが AI** です。今回も AI は、人間が用意した SSH 鍵と  を使って作業しています。
...
そして、この構築記録が残っていること自体が資産になります。**最終的な構成（・ジョブ・設定）はリポジトリに、OS 側で何をしたか・どこで詰まったかは本記事に残っています**。1 年後に同じ構築をするとき、あるいは担当が替わったときに辿れる形になっているわけです — 前節で書いた「安定するほど忘れる」への、ささやかな対策でもあります。

逆に言うと、**Git のコミット履歴に残るのはファイルの変更だけ**で、 や  のような OS 側の操作は、意識して文書に残さないと消えます。**構築ログをどこに残すかも「決めごと」の 1 つ**です。
