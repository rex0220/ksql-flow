...
環境は ConoHa VPS（1GB / 2Core・**検証時 月 1,064 円**）+ Ubuntu 24.04 LTS。**素の状態から通しでセットアップし、失敗と復旧まで検証**しています。そして例によって、その過程で罠を 5 つ踏みました。
...
| 運用の手間 | OS 管理は既存の運用に相乗り | ほぼゼロ | **OS 管理を自分たちで引き受ける**（更新・監視） |
| PR 検証 | なし | あり | GitHub Actions と**併用可**（後述） |

VPS が向くのは「**社内サーバーは無い・時刻は正確に・OS を管理する体制がある**」ケースです。管理の担い手は組織によって違います — **情シスがあるなら情シスの管理下に置くのが自然**で、個人や小規模チームなら自分たちで持つことになります。逆に、時刻が多少ずれてもよいなら #6 のほうが管理コストゼロで優れています。ここは正直に選んでください。
...
すでに VPS を持っているなら、そこに同居させたくなります。**まず見るのはメモリですが、`free` 列ではなく `available` 列と、既存サービスの実使用量・スワップの状況**を合わせて見ます。

実際、私は別の VPS（2GB/3Core）で公開中の Web サービスを動かしており、最初はそこに載せようとしました。状態はこうでした:

```bash
free -h
#                total   used   free   shared  buff/cache  available
#   Mem:          1.9Gi  1.4Gi  141Mi     35Mi      603Mi      540Mi
#   Swap:         3.0Gi  825Mi  2.2Gi

ps aux --sort=-%mem | head -5
#   アプリケーションサーバーのワーカー × 4（各 260〜390MB）+ 非同期ジョブ処理（326MB）
```

`free` 列は 141MB しかありませんが、**新しいプロセスを起動できる余力を表すのは `available`（540MB）のほう**です（`buff/cache` は必要なら解放されるため）。数字だけ見れば kSQL Flow のピーク RSS（本連載の実測で 200MB 超）は入りそうに見えます。

判断を分けたのは**スワップを 825MB 使っている**ことでした。既存サービスだけで**すでにメモリ圧が掛かっている**状態で、そこへ 200MB 級のプロセスを毎朝乗せれば、**公開中のサイトがスワップに追いやられる（最悪は OOM で落ちる）**リスクがあります。結局、検証用に別の VPS（1GB）を立てました。

これはあくまで **1 例**です（メモリを多く使うアプリケーションだったという事情があります）。目安としては:

- **`available` が 500MB 程度以上**あることを一つの目安にする（kSQL Flow のピーク + 余裕。ただしこれは本連載の実測から作った**経験則**で、ジョブの規模によって変わります）
- **スワップの使用量も見る** — `available` が足りていても、既にスワップを使っているなら余裕はありません
...
**fail2ban の「jail（ジェイル）」**は、この監視 1 つ分の設定単位です。「**どのログを見て・どんな失敗を数え・何回で・どれだけ遮断するか**」をひとまとめにしたもので、SSH 用の  jail が既定で有効になっています（Ubuntu の既定は概ね「10 分以内に 5 回失敗したら 10 分間その IP を遮断」。遮断は iptables などのファイアウォールに一時ルールを足す形で行われます）。今回の私は、この条件を軽々と超えました。
...
## この構築は AI（Claude Code）がやった

正直に書いておくと、**本記事のセットアップは、VPS を作った直後から cron 登録・障害ドリルまで、ほぼすべて AI（Claude Code）が SSH 経由で実行しました**。人間がやったのは 4 つだけです:

| 人間がやったこと | AI がやったこと |
| --- | --- |
| ConoHa で VPS を作成（OS・プラン・SSH Key の選択） | Node 導入・gh 導入・clone・`npm ci` |
| コンソールから SSH 公開鍵を登録（詰まった箇所の復旧） | 起動スクリプトの作成・配置・動作確認 |
| **ブラウザで gh のデバイス認証を承認** | cron 登録・自然発火の監視・ログ確認 |
| **`.env`（書込可トークン）を VPS へ転送** | 障害ドリル（ASSERT 違反 → 修正 → `--resume`）・後片付け |

[#2](https://qiita.com/rex0220/items/3a1213a596a8c49b67aa) では「AI がジョブ SQL を書き、人間が dry-run をレビューして本実行する」という分担でした。今回はその**インフラ版**です。そして分担の境界は #2 と同じ考え方になりました — **秘密を「決める・発行する・渡す」のが人間、与えられた権限の範囲で操作するのが AI** です。今回も AI は、人間が用意した SSH 鍵と `.env` を使って作業しています。

- **gh のデバイス認証**: コードは AI が取得できますが、承認はブラウザで人間が行う
- **書込可トークンの転送**: [#4](https://qiita.com/rex0220/items/d0a66c133edd42ff91c4) 以来の「無人サーバーの `.env` は人間が置く」方針どおり
- **VPS の作成と SSH 鍵の登録**: そもそもコンソールは人間しか触れません

逆に言うと、**それ以外は全部任せられました**。とくに効いたのは、本記事で挙げた罠に AI が実際にぶつかって回避した点です — dpkg ロックは待機ループを書き、鍵が貼れない問題は Gist 経由に切り替え、`git pull` の失敗から `gh auth setup-git` に辿り着き、`ssh.service disabled` を見て `ssh.socket` を確認しました。**本記事の「罠 5 つ」は、AI が踏んで直した記録そのもの**です。

一方で、**AI に任せて事故になった場面もありました**。SSH のユーザー名が分からず 14 通りを一気に試したところ、fail2ban に BAN されて自分たちを締め出しています。「短時間に何度も認証失敗すれば BAN される」のは基本ですが、機械は躊躇なく総当たりします。**コンソールという逃げ道を残しておくことの重要性**は、AI に作業させるならなおさらです。

そして、この構築記録が残っていること自体が資産になります。**最終的な構成（`run_batch.sh`・ジョブ・設定）はリポジトリに、OS 側で何をしたか・どこで詰まったかは本記事に残っています**。1 年後に同じ構築をするとき、あるいは担当が替わったときに辿れる形になっているわけです — 前節で書いた「安定するほど忘れる」への、ささやかな対策でもあります。

逆に言うと、**Git のコミット履歴に残るのはファイルの変更だけ**で、`apt install` や `systemctl` のような OS 側の操作は、意識して文書に残さないと消えます。**構築ログをどこに残すかも「決めごと」の 1 つ**です。

## コストの正直な話 — 月 1,064 円は総コストではない

> ここまでで「毎朝動かす」だけなら完成です。**以下は、この VPS を半年・1 年と運用する場合に発生する保守コストの話**になります。

ここまで「月 1,000 円」を前面に出してきましたが、**それは金銭コストだけ**です。実際の総コストには人の工数が乗ります。しかもそちらのほうが大きい。

| コスト | [#4](https://qiita.com/rex0220/items/d0a66c133edd42ff91c4) Windows サーバー | [#6](https://qiita.com/rex0220/items/a522f7880a5960f033b3) GitHub Actions | **#9 VPS + cron** |
| --- | --- | --- | --- |
| 金銭 | ゼロ（既存資産） | 追加費用ほぼゼロ（既存の無料/含有枠内なら） | 検証時 月 1,064 円 |
| 導入工数 | 約 15 分 | 約 10 分（Secrets 設定のみ） | **約 30 分**（本記事の実測） |
| **OS の保守** | 既存のサーバー運用に相乗り | **不要**（GitHub 任せ） | **自分たちで引き受ける**（更新・再起動・障害対応） |
| **鍵・認証の管理** | 既存のアカウント運用 | GitHub の Secrets 管理のみ | SSH 鍵・root パスワード・gh トークン |
| **手順書の保守** | 必要（構築・復旧手順） | 少ない（YAML がそのまま手順） | 必要（構築・復旧手順） |

金銭だけ見れば VPS がいちばん高く（といっても月 1,000 円です）、**保守工数も自分たち持ちになります**（情シスがある組織なら情シスの工数として乗ります）。逆に GitHub Actions は時刻がずれる代わりに、OS 更新も鍵管理も存在しません。つまり「月 1,000 円 vs 無料」という比較ではなく、**「時刻の正確さを、金銭ではなく保守工数で買っている」**と考えるのが正確です。
...
| パッチ・再起動ルール | 既存のサーバー運用に相乗り | 不要 | **自分たちで決める**（前述の方針 A/B/C） |
| 障害時の連絡・対応ルール | 既存の情シス窓口 | 同左 or 開発チーム | **誰が SSH で入るのか**まで決めておく |
| 監視・死活確認 | 既存の監視に相乗りできることが多い | Actions の失敗通知 | **自分たちで用意**（heartbeat・[#7](https://qiita.com/rex0220/items/39821af2a79b88de0ed2) の「起動しなかった事故」） |
...
**新しいカーネルと libc6 は適用済みなのに、動いているのは古いカーネル**です。`Unattended-Upgrade::Automatic-Reboot` は既定で `false` なので、**更新パッケージ自体は導入済みでも、カーネルのように再起動が必要な更新は、再起動するまで稼働環境へ完全には反映されません**（Ubuntu 24.04 では `needrestart` が影響するサービスを自動再起動するため、サービス単位の更新は反映されることがあります。カーネルと、既に起動しているプロセスが握るライブラリは別です）。「自動更新を有効にしたから安心」がいちばん危ない誤解で、`reboot-required` が何ヶ月も放置された VPS は珍しくありません。
...
実行時刻から十分離せば起きません（6:07 のバッチに対して再起動 4:00 なら 2 時間の間隔）。それでも重なった場合は、[#7](https://qiita.com/rex0220/items/39821af2a79b88de0ed2) の設計が保険になります — 部分適用で止まり、**翌日の実行で欠落を吸収できるジョブ（累積・全量型）なら冪等上書きで収束**します（[#8](https://qiita.com/rex0220/items/30cac8d8b52ec8ac782a) のクラス D と同じ構造。期間窓型ならバックフィルが要ります）。ロックが `RUNNING` のまま残っても、次回実行が `batchTimeoutSec` 超過を検知して自動回収します。
...
**逆に「自動化しない」リスク**も直視してください — 前節で見たとおり、`reboot-required` は**放置すると溜まり続け、脆弱性が当たっていない状態が何ヶ月も続きます**。「安定するほど忘れる」という前節の逆説がここでも効きます。①〜④ は技術的な対策を取りやすい一方、**手動運用の「忘れる」は長期になるほど制御しにくい**のです。
