...
[#2](https://qiita.com/rex0220/items/3a1213a596a8c49b67aa) では「AI がジョブ SQL を書き、人間が dry-run をレビューして本実行する」という分担でした。今回はその**インフラ版**です。そして分担の境界は #2 と同じ考え方になりました — **認証と秘密情報だけは人間**が持ちます。
...
そして、この構築記録が残っていること自体が資産になります。**やり取りはリポジトリのコミット履歴と本記事に残っているので、1 年後に同じ構築をするとき、あるいは担当が替わったときに「何をどの順でやったか」を辿れます** — 前節で書いた「安定するほど忘れる」への、ささやかな対策でもあります。
