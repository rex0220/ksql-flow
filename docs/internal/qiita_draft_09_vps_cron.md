...
なお「共有レンタルサーバーではどうか」もよく聞かれます。**さくらのレンタルサーバのような共有プランは難易度が高い**です — OS が FreeBSD で Node.js の公式バイナリが無く（ソースからのコンパイルが必要）、Docker も使えず、国外 IP フィルターが有効だと GitHub への通信も塞がれます。SSH と cron が使えても、その手前で詰まります。
...
**もう 1 つの教訓**: ユーザー名が分からず `root` / `ubuntu` / `admin`… と総当たりで試したところ、**fail2ban に BAN されて自分が締め出されました**。SSH の認証失敗は BAN 対象です。**コンソールという逃げ道が常にある**ことだけは覚えておいてください（BAN の解除も `fail2ban-client set sshd unbanip <IP>` でコンソールから可能です）。
...
- **cron の遅延は実測 1 秒**。[#6 の GitHub Actions（+30〜99 分）](https://qiita.com/rex0220/items/a522f7880a5960f033b3)との差は、時刻に締切があるジョブでは決定的
- セットアップは素の Ubuntu から **約 30 分**（Node 導入 → clone → `.env` → cron）。メモリ使用は 357MB で、1GB プランに余裕をもって収まる
