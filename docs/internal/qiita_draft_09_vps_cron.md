...
実際、私は別の VPS（2GB/3Core）で Discourse のコミュニティサイトを公開しており、最初はそこに載せようとしました。しかし状態を見ると:

```bash
free -h
#   total 1.9Gi / used 1.8Gi / free 74Mi     ← 空き 74MB
ps aux --sort=-%mem | head -5
#   unicorn worker × 4（各 260〜390MB）+ sidekiq（326MB）
```
...
これはあくまで**1 例**です（Discourse は比較的メモリを食うアプリケーションで、同居先が軽いサービスなら話は変わります）。判断基準としては:
