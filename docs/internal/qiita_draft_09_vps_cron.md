...
| **OS の保守** | 既存の情シス運用に相乗り | **不要**（GitHub 任せ） | **自分（更新・再起動・障害対応）** |
| **鍵・認証の管理** | 既存のアカウント運用 | GitHub の Secrets 管理のみ | SSH 鍵・root パスワード・gh トークン |
| **手順書の保守** | 必要（構築・復旧手順） | 少ない（YAML がそのまま手順） | 必要（構築・復旧手順） |

金銭だけ見れば VPS がいちばん高く（といっても月 1,000 円です）、**保守工数も自分に来ます**。逆に GitHub Actions は時刻がずれる代わりに、OS 更新も鍵管理も存在しません。つまり「月 1,000 円 vs 無料」という比較ではなく、**「時刻の正確さを、金銭ではなく保守工数で買っている」**と考えるのが正確です。
