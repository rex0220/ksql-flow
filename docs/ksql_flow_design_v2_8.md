...
* `ksql init-logapp --profile prod` で、8.2 のフィールド定義を持つログアプリを自動作成（kintone アプリ作成 API 使用）し、`job_key` フィールドの重複禁止設定まで行う。
* 既存アプリを使う場合は `ksql validate --check-logapp` でフィールド定義の過不足を検査。
