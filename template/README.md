...
5. **orchestrator（kSQL-FlowNet）で使う場合のみ**: 作成したアプリの画面で `scripts/logapp_v04_field_acl.console.js` を Console 実行し、相関 5 フィールドを Everyone=閲覧のみ にする（テンプレートはフィールドアクセス権を持ち出せないため、この 1 手順だけはスクリプトで行う。ランナーの API トークン書込には影響しない）。ポーラーも導入する場合は my-ksql-jobs の `dev/set_rerun_field_acl.console.js`（全編集可能フィールド対象・相関フィールドも自動的に含む）でもよい
6. **空値の保持確認**: `job_key` の重複禁止を設定したあと、**空の `job_key` を持つレコードを 2 件保存できることを確認**してください（テンプレートから作成した場合も念のため）。ロックの解放は「終了時に `job_key` を空にする」ことで成立しており、空値が複数保持できない環境では本バージョンのロック機構は動作しません（この挙動は kintone の公式ドキュメントに明記のない観測された仕様です — 破れた場合は fail-closed で全実行が停止します）
